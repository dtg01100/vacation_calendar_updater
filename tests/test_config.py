...
def test_config_defaults_created(tmp_path):
    ...
    settings = manager.ensure_defaults(default_email="user@example.com", calendar_options=["cal1", "cal2"])
    assert settings.email_address == "user@example.com"
    assert settings.calendar == "cal1"
    assert settings.send_email is True
    # persisted
    loaded = ConfigManager(path=cfg_path).ensure_defaults(default_email="user@example.com", calendar_options=["cal1"])
...
def test_config_save_roundtrip(tmp_path):
    ...
    settings = Settings(
        ...
        calendar="cal2",
        weekdays={"monday": False, "tuesday": True, "wednesday": False, "thursday": True, "friday": False, "saturday": False, "sunday": False},
        send_email=False,
    )
    manager.save(settings)
    loaded = manager.ensure_defaults(default_email="x@example.com", calendar_options=["cal2"])
