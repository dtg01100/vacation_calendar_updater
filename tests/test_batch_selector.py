...
class TestGetBatchesForDate:
    ...
    def test_get_batches_for_date_basic(self, undo_manager):
        ...
        batch_tomorrow = create_test_batch(batch_id="tomorrow", start_offset_days=1)
        batch_far = create_test_batch(batch_id="far", start_offset_days=15)
    ...
    def test_get_batches_for_date_boundary(self, undo_manager):
        ...
        batch_7_days_ago = create_test_batch(batch_id="7_ago", start_offset_days=-7)
        batch_7_days_future = create_test_batch(batch_id="7_future", start_offset_days=7)
        batch_8_days_ago = create_test_batch(batch_id="8_ago", start_offset_days=-8)
    ...
    def test_get_batches_for_date_multiple_events(self, undo_manager):
        ...
        today = dt.date.today()
        batch = create_test_batch(batch_id="multi", event_count=3, start_offset_days=0)
    ...
    def test_get_batches_for_date_excludes_undone(self, undo_manager):
        ...
        today = dt.date.today()
        batch_active = create_test_batch(batch_id="active", start_offset_days=0, is_undone=False)
        batch_undone = create_test_batch(batch_id="undone", start_offset_days=0, is_undone=True)
        
        undo_manager.undo_stack.append(batch_active)
        undo_manager.undo_stack.append(batch_undone)
        
        batches = undo_manager.get_batches_for_date(today, day_range=7)
        batch_ids = [b.batch_id for b in batches]
        
        assert "active" in batch_ids
        assert "undone" not in batch_ids
    
    def test_get_batches_for_date_custom_range(self, undo_manager):
        """Test with custom day_range."""
        today = dt.date.today()
        batch_3_days = create_test_batch(batch_id="3_days", start_offset_days=3)
    ...
    def test_get_batches_for_date_legacy_batches(self, undo_manager):
        """Test that legacy undo_history batches are included."""
        today = dt.date.today()
        
        batch_legacy = create_test_batch(batch_id="legacy", start_offset_days=0)
        undo_manager.undo_history.append(batch_legacy)
        
        batches = undo_manager.get_batches_for_date(today, day_range=7)
        batch_ids = [b.batch_id for b in batches]
        
        # Should get legacy batch when querying
        assert "legacy" in batch_ids
