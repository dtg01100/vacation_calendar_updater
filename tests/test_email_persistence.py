...
def test_email_persisted_when_api_updates_it(tmp_path):
    ...
    initial_settings = config_manager.ensure_defaults(
        default_email="",
        calendar_options=["Primary"]
    )
    assert initial_settings.email_address == ""
    
    # Simulate what happens when API provides email
    # (like in _on_startup_finished)
...
def test_user_email_not_overwritten_by_api(tmp_path):
    ...
    # This is what _on_startup_finished does now (fixed):
    ...
    final = config_manager.ensure_defaults(
        default_email="",
        calendar_options=["Work"]
    )
    assert final.email_address == "custom@example.com", "User's custom email should not be overwritten by API"
