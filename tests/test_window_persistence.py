...
def test_save_settings_to_disk_persists_email_and_calendar(tmp_path):
    ...
    loaded = config_manager.ensure_defaults(
        default_email="", calendar_options=["OldCalendar", "NewCalendar"]
    )
    ...
    loaded2 = config_manager2.ensure_defaults(
        default_email="", calendar_options=["OldCalendar", "NewCalendar"]
    )
...
def test_config_persistence_roundtrip_multiple_times(tmp_path):
    ...
    for email, calendar in test_cases:
        ...
        # Reload and verify
        loaded = config_manager.ensure_defaults(
            default_email="", calendar_options=["Primary", "Work", "Personal"]
        )
        assert loaded.email_address == email, (
            f"Expected {email}, got {loaded.email_address}"
        )
        assert loaded.calendar == calendar, (
            f"Expected {calendar}, got {loaded.calendar}"
        )
